...
/**
 * Utility class containing functions for generating random values of various types. All functions
 * are configurable with parameters to control the range, length, or other characteristics of the
 * generated values.
 */
object ObjektRandom {
   ...
   * @return A random string with length between minLength and maxLength
   */
  fun string(minLength: Int = 5, maxLength: Int = 20, charPool: String): String {
  ...
  fun alphanumericString(minLength: Int = 5, maxLength: Int = 20): String {
    return string(
        minLength, maxLength, ('a'..'z').toList() + ('A'..'Z').toList() + ('0'..'9').toList())
  }
   ...
   */
  fun <T> list(minSize: Int = 1, maxSize: Int = 10, generator: () -> T): List<T> {
   ...
   */
  fun <T> set(minSize: Int = 1, maxSize: Int = 10, generator: () -> T): Set<T> {
  ...
  fun <K, V> map(
    ...
    // Keep generating key-value pairs until we reach the target size or exceed a reasonable attempt
    // limit
    var attempts = 0
    while (result.size < targetSize && attempts < targetSize * 3) {
